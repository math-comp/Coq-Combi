...
Proof. by elim: s => //= s0 s; case: (rem_trail0 s) => //= _; case: eqP. Qed.

Lemma nth_rem_trail0 s i : nth 0 (rem_trail0 s) i = nth 0 s i.
Proof.
elim: s i => //= s0 s.
case: (rem_trail0 s) => [/= | t1 t'] IHs i.
- by case (s0 =P 0) => [-> | _]; case: i => //= i; rewrite -IHs.
...
by case: (s0 =P 0%N) => [-> | _] /=; rewrite ?addn0.
Qed.
...
Proof.
elim: sh i => [| sh0 sh IHsh] /= i.
  by move=> _; case: i => //= i /andP[]; rewrite leqn0 => /part_head0F ->.
case: i => // i.
...
case (i =P 0) => [->{i} _ | /eqP Hi /= H]; apply/is_partP.
...
- case: i Hi H => [//= | i] _; split; first exact: last_incr_nth_non0.
  ...
  + rewrite !nth_incr_nth; case (i =P j) => Hi /=.
...
Proof. by elim: n => //= [] [|n] // /(_ (ltn0Sn n))->. Qed.
...
Proof.
by case: p1 => //= a l; case: p2 => //= b s /andP[].
Qed.
...
by case=> //= out0 out; rewrite subSS IHinn.
Qed.
...
HB.instance Definition _ := [isSub of intpart for pval].
HB.instance Definition _ := [Countable of intpart by <:].
...
HB.instance Definition _ := [isSub of intpartn for pnval].
HB.instance Definition _ := [Countable of intpartn by <:].
...
HB.instance Definition _ d := isInhabitedType.Build (intpartn d) (rowpartn d).
...
Proof.
rewrite leEintpartnlexi leEseqlexi.
...
HB.instance Definition _ := [isSub of box_skew for box_skewval].
HB.instance Definition _ := [Countable of box_skew by <:].
...
HB.instance Definition _ := Finite.copy box_skew
  (uniq_finType (box_skew : subCountType _) mem_enum_box_skew enum_box_skew_uniq).
...
rewrite is_part_sortedE; apply/andP; split; first last.
  apply/negP => /(nthP 0) [i]; rewrite nth_meet_Young.
...
Proof. by apply/le_YoungP => i; rewrite nth_meet_Young geq_minl. Qed.
...
by apply/le_YoungP => i; rewrite nth_meet_Young leq_min le1 le2.
Qed.
...
Proof.
rewrite is_part_sortedE; apply/andP; split; first last.
  apply/negP => /(nthP 0) [i]; rewrite nth_join_Young.
...
Proof. by apply/le_YoungP => i; rewrite nth_join_Young leq_maxl. Qed.
