...
have /set0Pn[x Bx] := memPn notP0 B HB.
...
rewrite (eq_filter (a2 := (fun i : 'I_m => preim (nth u s) P i))) => [i|].
  by rewrite inE unfold_in.
...
HB.instance Definition _ := Equality.copy brace (can_type bool_of_braceK).
HB.instance Definition _ := Choice.copy brace (can_type bool_of_braceK).
HB.instance Definition _ := Countable.copy brace (can_type bool_of_braceK).
HB.instance Definition _ := Finite.copy brace (can_type bool_of_braceK).
...
Proof. by elim: l => //= w l IHl /andP[Hd /IHl]; apply: Dyck_word_cat. Qed.
...
HB.instance Definition _ := [isSub of Dyck for dyckword].
HB.instance Definition _ := [Countable of Dyck by <:].
...
have {}Hmin n : (n <= cut)%N -> height (take n tl) >= 0.
  ...
  apply/andP; split; first by rewrite lez1D.
...
have HDL : (take cut tl) \is a Dyck_word.
  ...
  rewrite size_take Hcut => Hncut.
  by rewrite take_takel // Hmin.
...
have Dyck_fact_size (d1 d2 : Dyck) :
  ...
  - move: Heq => /eqP {Hmin}; rewrite leqNgt; apply contraL; rewrite ltnS.
    ...
    + rewrite !height_simpl /=.
      ...
      by apply lt0r_neq0; rewrite ltz1D.
...
rewrite (eq_ex_minn _ (Dyck_cut_ex (join_Dyck_nnil E1 E2))) => [i|].
  by rewrite Heq.
...
case: (D =P nil_Dyck) => [-> // | /eqP/factor_Dyck [[D1 D2] /= ->]].
...
- rewrite -[i]/(nat_of_ord (Ordinal ilt)).
  rewrite lerNl opprB subzn // lez_nat.
  exact: leq_bigmax.
- move=> /ltnW.
...
set F := BIG_F; move/(eq_bigmax_cond F) => [[i Hi _]]; rewrite {}/F /= => Hmax.
...
Proof.
rewrite oppr_lt0 gtz0_ge1 -lerN2 -Hbal1.
...
- have -> : size (take (size w).-1 (rot pfminh w)) = (size w).-1.
  ...
  + have {Hnpf} : (n - (size w - pfminh) < pfminh)%N.
      rewrite subnBA ?pfminh_size // addnC -subnBA ?leq_subr.
    ...
    rewrite height_simpl take_takel; first exact: ltnW.
...
- rewrite /rot !take_cat !size_drop !size_rcons.
  ...
  + rewrite take_drop !height_drop take_takel ?leq_addl //.
    rewrite lerBDr subrK -cats1 takel_cat //.
    rewrite !height_simpl /= addr0.
    move: HDyck => /Dyck_wordP[H1 ->].
    rewrite add0r; apply: (le_trans _ (H1 _)).
    by rewrite -oppr_ge0 opprK.
  + rewrite height_simpl addrC -lerBlDr subrr.
    case: (leqP (i - (size w - rt).+1) rt) => [H | /ltnW H].
    * rewrite take_takel // -cats1 takel_cat; first exact: leq_trans H Hrt.
...
Proof.
have := card_draws ('I_(n.*2) : finType) n; rewrite card_ord => <-.
...
- exists (mktuple (fun i => if i \in S then }} else {{)); first last.
  ...
  rewrite [X in (Posz (size X) - _)%R](eq_filter (a2 := mem (~: S))) => [i|].
    by rewrite /= in_setC; case: (i \in S).
  rewrite [X in (_ - Posz (size X))%R](eq_filter (a2 := mem S)) => [i /=|].
    by case: (i \in S).
...
- rewrite (eqP size_UnDn) take_cat !size_nseq => Hi2.
  ...
  + rewrite height_simpl take_nseq /=; first by rewrite leq_subLR addnn.
...
- move=> i; rewrite size_rcons ltnS => Hi.
  rewrite -cats1 (takel_cat _ Hi) !height_simpl /=.
  move: HD => /Dyck_wordP[Hpos ->].
  by rewrite addr0 add0r -subr_gt0 opprK ltzD1.
Qed.
...
Proof.
case: w => [w _] /= Hsz H H0.
rewrite /rot take_cat size_drop size_rcons subKn; first exact: (leq_trans Hsz).
...
rewrite !size_drop subKn; first by apply: (leq_trans Hsz); rewrite size_rcons.
rewrite ltnNge Hsz /= nth_take; first by rewrite size_rcons subSn.
by rewrite Hk.
Qed.
...
rewrite pfminh_rrw // size_rcons; first by rewrite subSS leq_subr.
rewrite subKn; first exact: (leq_trans Hsz).
by rewrite rotrK.
Qed.
...
rewrite (@rot_pfminhE (rcons w }})).
  by rewrite height_simpl /= Hbal add0r.
...
- have := Dyck_of_balK ubal; rewrite Huw => ->.
  ...
  + exists (Ordinal (Hpfminh)); last by [].
    ...
    rewrite take_cat size_drop subKn ?size_rcons //; first exact: ltnW.
...
rewrite card_in_imset.
  ...
  - move=> _ Hnth _; case: j Hj Hnthj => // j Hj Hnthj.
    ...
    rewrite !pfminh_rrw //= size_rcons; first by rewrite subSS leq_subr.
    rewrite subn0 subKn ?ltnS; first exact: ltnW.
...
have -> : n.+1 = count_mem }} (rcons D }}).
  ...
  by move: HD => /Dyck_wordP[].
rewrite -(@card_preim_nth (size D).+1 _ _ _ {{); first by rewrite size_rcons.
congr (#|pred_of_set _|).
