...
have Hv0 : v0 < size v by exact: leq_ltn_trans (leq_addl _ _) Hv1.
have -> : v = (take v0 v) ++
...
have -> : size s - m.+1 = head 0 (drop m s).
  rewrite -nth0 nth_drop addn0; apply (@addIn m.+1).
  rewrite subnK // addnS.
  apply anti_leq; rewrite Hnth /=.
...
Lemma nth_add_ltE {u v0 v r} :
  r < size u ->
  nth 0 (u ++ v0 :: v) r = nth 0 (u ++ (nth 0 v v0 + v0).+1 :: v) r.
Proof. by rewrite !nth_cat => ->. Qed.
Lemma nth_add_gtE {u v0 v r} :
  r > size u ->
  nth 0 (u ++ v0 :: v) r = nth 0 (u ++ (nth 0 v v0 + v0).+1 :: v) r.
Proof.
move=> Hr; rewrite !nth_cat [r < _]ltnNge (ltnW Hr) /=.
by move: Hr; rewrite -subn_gt0; case: (r - size u).
Qed.

Lemma root_leq_add u v0 v :
  ex_minn (root_proof (u ++ (nth 0 v v0 + v0).+1 :: v)) <=
  ex_minn (root_proof (u ++ v0 :: v)).
Proof.
case: ex_minnP => r0; rewrite size_cat /= addnS => Hr01 Hr02.
case: ex_minnP => r;  rewrite size_cat /= addnS => Hr1  Hr2.
apply Hr02; case: (ltngtP r (size u)) => Hr.
- by rewrite -(nth_add_ltE Hr).
- by rewrite -(nth_add_gtE Hr).
- apply: (leq_trans Hr1); rewrite !nth_cat /= Hr ltnn subnn /=.
  by rewrite ltnS -addSn -addnA leq_addl.
Qed.

Lemma rotations_add_alt u v0 v :
  u ++ v0 :: v \is a TamariVector ->
  u ++ (nth 0 v v0 + v0).+1 :: v \is a TamariVector ->
  from_rsz (u ++ (nth 0 v v0 + v0).+1 :: v)
           \in rotations (from_rsz (u ++ v0 :: v)).
Proof.
move: {2}(size u + size v).+1 (ltnSn (size u + size v)) => n.
elim: n u v0 v => [|n IHn] u v0 v; first by case u.
rewrite ltnS => Hsz Htam Htam0.
have Hnnil : u ++ v0 :: v != [::] by case u.
have Hnnil0 : u ++ (nth 0 v v0 + v0).+1 :: v != [::] by case u.
rewrite from_rszE // [X in rotations X]from_rszE // !Tamari_root //.
set root0 := ex_minn _; set root := ex_minn _.
case: (ltnP root (size u)) => Hroot.
  have Hreq : root = root0.
    move: Hroot; rewrite {}/root0 {}/root.
    have:= root_leq_add u v0 v.
    case: ex_minnP => r0; rewrite size_cat /= addnS => Hr01 Hr02.
    case: ex_minnP => r;  rewrite size_cat /= addnS => Hr1  Hr2 Hr0r Hr.
    apply anti_leq; rewrite Hr0r andbT.
    rewrite -(nth_add_ltE (leq_ltn_trans Hr0r Hr)) in Hr01.
    exact: Hr2.
  have:= Tamari_drop root0.+1 Htam.
  have:= Tamari_drop root0.+1 Htam0.
  rewrite -Hreq !take_cat Hroot !dropl_cat // => {Htam Htam0} Htam Htam0.
  apply: rotation_right_sub.
  apply: IHn => //; apply: (leq_trans _ Hsz).
  rewrite size_drop ltn_add2r -(ltn_add2r root.+1) subnK //.
  by rewrite addnS ltnS leq_addr.
case: (leqP root0 (size u)) => Hroot0.
  have {Hroot0 IHn Hsz} Hroot0 : root0 = size u.
    apply: anti_leq; rewrite Hroot0 /=.
    move: Hroot0 Hroot; rewrite {}/root0 {}/root.
    case: ex_minnP => r0; rewrite size_cat /= addnS => Hr01 Hr02.
    case: ex_minnP => r;  rewrite size_cat /= addnS => Hr1  Hr2 Hr0r Hr.
    rewrite leqNgt; apply (introN idP) => {Hr0r} Hr0r.
    move: Hr01; rewrite -(nth_add_ltE Hr0r) =>/Hr2/(leq_trans Hr)/(leq_trans Hr0r).
    by rewrite ltnn.
  move: Hroot0; rewrite {}/root0.
  case: ex_minnP => r0 Hr01 Hr02 Hr0.
  rewrite size_cat /= addnS ltnS in Hr01 Hr02.
  subst r0.
  rewrite take_size_cat // -cat1s catA drop_size_cat; first last.
    by rewrite size_cat /= addn1.
  have {Hroot Hr01 Hr02} Hroot : root = (v0 + size u).+1.
    subst root; case: ex_minnP Hroot => r.
    rewrite size_cat /= addnS => Hr1 Hr2 Hr.
    apply anti_leq; apply/andP; split.
    * apply Hr2.
      rewrite ltnS nth_cat ltnNge -addSn leq_addl /= addnK /=.
      by move: Hr01; rewrite nth_cat ltnn subnn /= !addSn addnS addnA.
    * have Hult: size u < (size u + size v).+1 by rewrite ltnS leq_addr.
      have {Hr} Hr : size u < r.
        rewrite ltnNge; apply (introN idP) => Hr'.
        have {Hr Hr'} Hr : r = size u by apply anti_leq; rewrite Hr Hr'.
        subst r.
        move: Htam0 => /TamariP []; rewrite size_cat /= addnS => Htam0 _.
        move/Htam0/leq_trans/(_ Hr1): Hult.
        rewrite !nth_cat ltnn subnn /= addSn ltnS.
        by rewrite ltnNge -addnA leq_addl.
      rewrite ltnNge; apply (introN idP) => Hrv0.
      move: Htam => /TamariP []; rewrite size_cat /= addnS => Htam1 Htam2.
      have {Htam2} /Htam2 : size u < r <= nth 0 (u ++ v0 :: v) (size u) + size u.
        by rewrite Hr /= nth_cat ltnn subnn.
      rewrite [in X in _ <= X]nth_cat ltnn subnn /=.
      move: Hr1; rewrite ltnS => /leq_trans H/H{H}.
      move: Htam0 => /TamariP []; rewrite size_cat /= addnS => Htam0 _.
      move/(_ _ Hult): Htam0; rewrite ltnS nth_cat ltnn subnn /=.
      move=> /leq_trans H/H{H}.
      by rewrite addSn ltnNge -addnA leq_addl.
  rewrite {}Hroot {root}.
  have -> : take (v0 + size u).+1 (u ++ v0 :: v) = u ++ v0 :: take v0 v.
    by rewrite take_cat ltnNge -!addSn leq_addl /= addnK.
  have -> : drop (v0 + size u).+2 (u ++ v0 :: v) = drop v0.+1 v.
    by rewrite drop_cat ltnNge -!addSn leq_addl /= addnK.
  have -> : from_rsz (u ++ v0 :: take v0 v) =
                      BinNode (from_rsz u) (from_rsz (take v0 v)).
    admit.
  have -> : from_rsz v = BinNode (from_rsz (take v0 v)) (from_rsz (drop v0.+1 v)).
    move: Htam => /(Tamari_drop (size u)).
    rewrite drop_cat ltnn subnn drop0 => Htam.
    from_rszE
