...
- elim: outer inner Hstrip Hout Hinn => [| out0 out IHout] //= [| inn0 inn].
  ...
  + move=> /andP[/eqP-> {out0 IHout} H] _ _ [|i] //=.
    ...
    by move=> /andP[/eqP-> {}/IHout Hrec]; case.
...
- elim: outer inner Hout Hinn => [| out0 out IHout].
  ...
  + move=> inner Hpart; have:= part_head_non0 Hpart => /=.
    ...
    * move: Hpart => /andP[H0 {}/IHout Hrec] /andP[_ {}/Hrec Hrec] H.
      have := H 0 => /= -> /=.
      apply Hrec => i.
      exact: H i.+1.
Qed.
...
#[local] Abbreviation res := (add_ribbon_on sh start stop rem).
...
#[local] Abbreviation box := (box_skew inner outer).
...
#[local] Abbreviation neig4 := (grel (@neig4box inner outer)).
...
#[local] Abbreviation boxex := ribbon_on_box_ex.
...
#[local] Abbreviation box := (box_skew inner outer).
#[local] Abbreviation stop := (mindropeq inner outer).-1.
#[local] Abbreviation neig4 := (grel (@neig4box inner outer)).
...
#[local] Abbreviation start := ribbontb_start.
