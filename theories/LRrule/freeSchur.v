...
Proof using Htake. by rewrite /is_RStabpair is_tableau_RS Htake /= eq_refl andbT. Qed.
Lemma changeUdropP : is_RStabpair (RS (drop d1 w), val U2).
Proof using Hdrop. by rewrite /is_RStabpair is_tableau_RS Hdrop /= eq_refl andbT. Qed.
Lemma changeTtakeP : is_RStabpair (RS (take d1 w), val T1).
Proof using Hsh1 Htake. by rewrite /is_RStabpair is_tableau_RS Htake /= Hsh1 eq_refl andbT. Qed.
Lemma changeTdropP : is_RStabpair (RS (drop d1 w), val T2).
Proof using Hdrop Hsh2. by rewrite /is_RStabpair is_tableau_RS Hdrop /= Hsh2 eq_refl andbT. Qed.
...
Lemma card_LRtab_set_shapeE P (U1 T1 : stdtabn d1) (U2 T2 : stdtabn d2) :
  ...
  #|[set Q in (LRsupport U1 U2) | (shape Q == P)]| =
  #|[set Q in (LRsupport T1 T2) | (shape Q == P)]|.
Proof using . move=> H1 H2; apply anti_leq; rewrite !card_LRtab_set_leq // H1 H2. Qed.
