...
From mathcomp Require Import seq tuple finset perm fingroup path.
...
Lemma sorted_std_extract u (S : {set 'I_(size u)}) :
  ...
  sorted leqX (extractpred (in_tuple (std u))
...
Theorem RSmap_std (T : inhOrdType) (w : seq T) :
...
Corollary RStabmap_std (T : inhOrdType) (w : seq T) :
...
rewrite (set_nth_default (inhabitant _) _ Hpos) nth_posbig.
...
apply/linvseqP => i Hi.
rewrite (set_nth_default (inhabitant _) _ Hi) -nth_rembig.
...
rewrite (set_nth_default (size (rcons t tn)) (inhabitant _) Hpos).
