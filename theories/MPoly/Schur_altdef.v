...
case: (leqP s0 m0) => //= Hm0.
have Hs0 : s0 = m0.+1.
  by apply anti_leq; rewrite Hm0 sm0.
subst s0 => /= {Hm0}.
...
Lemma alt_straight_empty_rec lft (i : 'I_n) (d : nat) :
  i > size lft ->
  d - i >= size lft ->
  'a_(mpart lft + rho + U_(i) *+ d.+1) =
  (-1) ^+ (i - size lft) *:
       'a_(mpart (lft ++ nseq (i - size lft).+1 1) + rho +
           U_(inord (size lft)) *+ ((d - i) - size lft)).
Proof.
move=> Hi Hdi.
have Hszc : size (lft ++ nseq (i - size lft).+1 1) <= n.
  by rewrite size_cat size_nseq -subSn ?(ltnW Hi) // subnKC // ltnW // ltnW //.
have -> : mpart (lft ++ nseq (i - size lft).+1 1) =
          (mpart (lft) + \sum_(size lft <= j < i.+1) U_(inord j))%MM :> 'X_{1..n}.
  apply mnmP => j; rewrite !(mnmE, mpartE) //. nth_cat.
Admitted.

Lemma alt_straight_rec lft p pk (i : 'I_n) (d : nat) :
  is_part (lft ++ pk :: p) ->
  i > size lft ->
  forall res h rem,
    add_ribbon_base pk p d.+1 (i - size lft).-1 = Some (res, h, rem) ->
    'a_(mpart (lft ++ pk :: p) + rho + U_(i) *+ d.+1) =
    (-1) ^+ h.-1 *:
    'a_(mpart (lft ++ pk :: res) + rho + U_(inord (size lft)) *+ rem).
Proof.
Admitted.

Lemma alt_straight p (i : 'I_n) (d : nat) :
  is_part p -> size p <= n ->
  forall res h, add_ribbon p d.+1 i = Some (res, h) ->
    'a_(mpart p + rho + U_(i) *+ d.+1) = (-1) ^+ h.-1 *: 'a_(mpart res + rho).
Proof.
rewrite /add_ribbon => Hpart Hsz res h.
case Hrib: add_ribbon_base => // [[[x y] rem]] [Hx Hy]; subst x y.
have Hrem := add_ribbon_base_rem0 (is_part_base_init Hpart d) Hrib erefl.
rewrite Hrem in Hrib.
case: p Hpart Hsz Hrib => [_ _|p0 p]/=.
  case: ltnP => // leid.
  rewrite addn0 /add_ribcol subSn //= subn0 eqSS.
  case: eqP => // _ []; rewrite add0n minSS => <- <- /eqP.
  rewrite subSS subn_eq0 leq_min => /andP [_ ltdiff].
  rewrite mpart0 add0m (minn_idPl ltdiff).
  case: (altP (val i =P 0%N)) => /= eqi0.
    rewrite eqi0 expr0 scale1r subn0 addmC; congr ('a_(_ + rho)).
    apply mnmP => j; rewrite mpartE //= !mulmnE !mnm1E -val_eqE /=.
    case: (altP (_ =P _)) => [<-|]; first by rewrite eqi0 /= muln1.
    by rewrite muln0 eqi0 eq_sym -lt0n => Hj; rewrite nth_default.
  rewrite -[(rho + _)%MM]add0m -mpart0 addmA.
  rewrite alt_straight_empty_rec ?lt0n //= !subn0; congr (_ *: 'a_(_)).
  apply mnmP => j.
  rewrite !mnmE !mpartE /= ?size_nseq // ?(ltnW (ltn_ord _)) //.
  rewrite mulmnE !mnm1E -val_eqE /= inordK // addnC addnA; congr (_ + _).
  case: j => [[|j] /= _]; first by rewrite muln1 addn1.
  by rewrite muln0 add0n.
Admitted.

Lemma alt_straight_0 (p : seq nat) (i : 'I_n) (d : nat) :
  is_part p -> size p <= n ->
  add_ribbon p d.+1 i == None -> 'a_(mpart p + rho + U_(i) *+ d.+1) = 0%R.
Proof.
case: p => [|p0 p] Hpart /= Hsz; rewrite /add_ribbon /=.
  case: ltnP => [ltid _| ledi].
    have ltdi1 := (leq_ltn_trans (leq_subr d.+1 i) (ltn_ord i)).
    have gti : i > (i - d.+1).
      by rewrite ltn_subLR // addSn ltnS leq_addl.
    rewrite mpart0 add0m; set mon := (M in 'X_[M]).
    suff /alt_alternate -> : mon i = mon (inord (i - d.+1)) => //.
      by rewrite -val_eqE /= inordK // gtn_eqF.
    rewrite {}/mon !mnmE.
    rewrite !mulmnE !mnm1E eqxx -val_eqE /= muln1 inordK //.
    by rewrite (gtn_eqF gti) muln0 addn0 subnA // subn1 /= -ltnS.
  rewrite /add_ribcol /= subn_eq0 ltnNge ledi /= addn0 subn0.
  by rewrite ltn_eqF // ltnS leq_subr.
rewrite [p0 + _]addnC addnA.
case Hi : (nat_of_ord i) => [|i'].
  rewrite /add_ribcol /= ltn_eqF //.
  by rewrite -addSn addnK ltnS addSn ltnS leq_addr.
case Hrib: add_ribbon_base => [[[res h] rem]|].
  have := sumn_add_ribbon_base Hpart Hrib.
  rewrite /add_ribcol; case: rem Hrib => [|rem]//= _ Hsum.
  by rewrite -addSn addnK -{}Hsum -addSn -{1}(add0n rem.+1) eqn_add2r.
move=> _; move: Hpart Hsz Hrib; rewrite -{1 2 4}(cat0s p).
have {2}-> : p0 = last p0 [::] by [].
have -> : i' = i - size (T := nat) (p0 :: [::]) :> nat.
  by rewrite Hi subSS subn0.
have : size (T := nat) [::] < i by rewrite Hi.
elim: p d [::] => [|pk p IHp] d lft lfti Hbase /=.
  rewrite cats0 => Hsz; case: (ltnP d _) => [|Hi'].
    rewrite ltn_subRL => ltd _.
    set mon := (M in 'X_[M]).
    have ltdi := (leq_ltn_trans (leq_addl _ _) ltd).
    have ltdi1 := (leq_ltn_trans (leq_subr d.+1 i) (ltn_ord i)).
    have gti : i > (i - d.+1) by rewrite Hi subSS ltnS leq_subr.
    suff /alt_alternate -> : mon i = mon (inord (i - d.+1)) => //.
      by rewrite -val_eqE /= inordK // gtn_eqF.
    rewrite {}/mon !mnmE !mpartE //.
    rewrite !mulmnE !mnm1E eqxx -val_eqE /= muln1 inordK //.
    have Hszid : size lft < i - d.+1.
      by rewrite ltn_subRL addnC -addSnnS.
    rewrite !nth_default ?add0n //=.
    rewrite (gtn_eqF gti) muln0 addn0 subnA //.
    by rewrite Hi subn1 -ltnS /= -Hi.
  rewrite /add_ribcol !subn0; case: eqP => // _; case: eqP => //.
  rewrite subSn // => [][] Heq.
  set mon := (M in 'X_[M]).
  suff /alt_alternate -> : mon i = mon (Ordinal Hsz) => //.
    by rewrite -val_eqE /= gtn_eqF.
  rewrite {}/mon !mnmE !mpartE //.
  rewrite !mulmnE !mnm1E eqxx -!val_eqE /= muln1 gtn_eqF // muln0 addn0.
  rewrite nth_default //= add0n.
  rewrite -[size lft]/((size (p0 :: lft)).-1) nth_last /= -{}Heq.
  rewrite subn1 /=; rewrite ltnS in Hsz.
  rewrite addnBA // [_ + n0]addnC subnBA // -[d + _]addSnnS.
  rewrite addnBA; first last.
    by move: Hi'; rewrite addnC -leq_subLR Hi subSS subSn // -ltnS -Hi.
  rewrite -subnDA [i + size lft]addnC subnDA addnA addnK.
  by rewrite addnC addnBA ?[n0 + _]addnC // -ltnS.
move=> Hsz; case Hdiff : (i' - size lft) => [|j].
  move: Hdiff => /eqP {IHp}; rewrite subn_eq0 => lfti1.
  have {lfti1}lfti : i' = size lft by apply: anti_leq; rewrite lfti1 -ltnS -Hi.
  rewrite /add_ribcol //= Hi lfti subnn; case eqP => // Hd _.
  set mon := (M in 'X_[M]).
  have lti' : i' < n by rewrite -Hi; exact: ltnW.
  suff /alt_alternate -> : mon i = mon (Ordinal lti') => //.
    by rewrite -val_eqE /= Hi gtn_eqF.
  rewrite {}/mon !mnmE !mpartE //=.
  rewrite !mulmnE !mnm1E eqxx -!val_eqE /= muln1 gtn_eqF ?Hi// muln0 addn0.
  rewrite /= {1}lfti nth_cat ltnn subnn /= {}Hd.
  rewrite {2}lfti -[size lft]/((size (p0 :: lft)).-1).
  rewrite -/((p0 :: lft) ++ pk :: p) nth_cat nth_last /= ltnSn.
  have Hpk : pk <= last p0 lft.
    move: Hbase {Hsz lfti lti' Hi i' i}.
    by elim: lft p0 => /= [p0 /andP []//| s0 s IHs] p0 /andP[H0 H1]; exact: IHs.
  rewrite (subSn Hpk) addnC addSnnS -addnS addnA subnK //.
  rewrite subn1 /= -subSn ?subSS //.
  move: Hsz; rewrite lfti size_cat /= addnS ltnS.
  exact/leq_ltn_trans/leq_addr.
rewrite Hi subSS Hdiff.
case Hrib: add_ribbon_base  => [[[res h] rem]|].
  rewrite /add_ribcol {IHp}; case: eqP => // _; case: eqP => // Hrem _.
  rewrite -/((p0 :: lft) ++ pk :: p) in Hbase *.
  move: Hrib.
  have -> : j = (i - size (p0 :: lft)).-1.
    by rewrite Hi /= subSS Hdiff /=.
  move/alt_straight_rec -> => //; first last.
    by rewrite /= Hi ltnS -subn_gt0 Hdiff.
  rewrite /= [X in _ *: X](_ : _ = 0%R) ?scaler0 //.
  set mon := (M in 'X_[M]).
  suff /alt_alternate -> : mon (inord (size lft)) = mon (inord (size lft).+1) => //.
    admit.
  rewrite {}/mon !mnmE !mpartE //=.
  rewrite !mulmnE !mnm1E eqxx -!val_eqE /= muln1 gtn_eqF ?Hi//.
  rewrite muln0 addn0 !inordK.
  rewrite -/((p0 :: lft) ++ pk :: res) !nth_cat /= ltnSn ltnn subnn /=.
  rewrite -[size lft]/((size (p0 :: lft)).-1) nth_last /=.
  rewrite {}Hrem [RHS]addnC addnA subnK.
  rewrite [_ - (size lft).+1]subnS addSnnS.
  (*
  have := sumn_add_ribbon_base (is_part_catl Hbase) Hrib.
  rewrite {}Hrem.
  *)
  admit.
move=> _.
have Hlt : size lft < i' by rewrite ltnNge -subn_eq0 Hdiff.
move: Hbase Hsz;rewrite -(cat1s pk) catA => {}/IHp H{}/H /=; apply.
  by rewrite size_cat /= addn1 Hi ltnS.
