...
Proof.
rewrite /al_length /is_in_shape /leg_length /arm_length =>
  /is_part_ijP [] _ Hpart Hr Hc.
...
Lemma al_length_ltl sh r c j :
  is_part sh -> c < j -> is_in_shape sh r j ->
  al_length sh r j < al_length sh r c.
Proof.
...
Lemma al_length_ltr sh r c j :
  is_part sh -> r < j -> is_in_shape sh j c ->
  al_length sh j c < al_length sh r c.
Proof.
rewrite /al_length => Hpart Hr Hc.
...
Lemma al_length0_corner sh r c :
  ...
  al_length sh r c = 0 -> is_corner_box sh r c.
Proof.
move => Hpart.
...
by rewrite !addnA; congr (_ + _); rewrite -!addnA; congr (_ + _); rewrite addnC.
Qed.
...
- move=> /andP [] /eqP HA /eqP HB; subst.
  ...
  rewrite /is_corner_box /al_length /leg_length /arm_length /is_rem_corner.
...
Lemma reshape_coord_walk_to :
  ...
  \sum_(X <- enum_trace Alpha Beta |
        let (r, c) := reshape_coord p i in starts_at r c X)
   PI_trace X.
Proof using Hcorn.
...
rewrite (eq_bigr F); first last.
  ...
  rewrite /PI_trace -(PIprog (m := al_length p (head O A) (head O B))) /=;
      first last.
...
rewrite -big_filter (bigD1_seq (flatten_coord p (head O A) (head O B))) /=;
    first last.
...
rewrite -mulrA.
rewrite -HBeta' (corner_al_length0 (intpartP p) Hcorn) mul1r.
rewrite -prodf_div /=.
